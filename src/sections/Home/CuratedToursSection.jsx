...
import bgPalmLeaf from '../../assets/images/icons/palmLeafBg.svg'; // Decorative background leaf
...
const TourCard = ({ image, title, category, details }) => (
    <div className="bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:-translate-y-2 flex flex-col">
        <img src={image} alt={title} className="w-full h-56 object-cover" />
        <div className="p-6 flex flex-col flex-grow">
            <h3 className="font-[var(--font-teachers)]">{title}</h3>
            <p className="text-sm text-stone-500 font-sans tracking-wide mt-1">{category}</p>
            <div className="mt-4 border-t border-stone-200 pt-4 space-y-3 font-sans text-sm">
                {details.map((detail, index) => (
                    <div key={index} className="flex justify-between items-center text-stone-600">
                        <span className="font-semibold text-stone-800">{detail.label}</span>
...
const CuratedToursSection = () => {
    return (
        <section className="bg-[#FBF9F3] font-serif py-20 md:py-28 relative overflow-hidden">
